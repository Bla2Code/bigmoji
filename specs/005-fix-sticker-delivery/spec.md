...
### User Story 2 - Send sticker as the original message author with "БОТ" suffix (Priority: P2)

As a Discord user, I want the sticker response to appear as if it was sent by me (the original message author) with "БОТ" appended to my name, so that the sticker feels like a personal reaction rather than a bot response.
...
1. **Given** a user named "Alice" sends a message with a supported emoji, **When** the listener processes the message, **Then** the sticker is sent via webhook with username "Alice БОТ" and the original user's avatar.
2. **Given** a user with a special character name sends a message, **When** the listener processes the message, **Then** the webhook username is sanitized to comply with Discord's username rules (max 32 chars, no @/#).
3. **Given** the webhook creation fails (e.g., missing permissions), **When** the listener processes the message, **Then** the system falls back to sending the sticker under the bot's own name and logs a warning.
...
- **FR-003**: System MUST create or reuse a Discord webhook in the target channel to send the sticker under the original author's name with "БОТ" suffix.
- **FR-004**: System MUST set the webhook username to `<original_author_name> БОТ`, sanitized to comply with Discord's username rules (max 32 characters, no `@` or `#`).
...
- **Webhook Username**: Sanitized version of the original author's display name with "БОТ" suffix, max 32 characters.
...
- **SC-002**: 100% of sticker responses appear under the original author's name with "БОТ" suffix when webhook permissions are available.
...
- The original message author's display name is accessible via `event.getAuthor().getName()` or `event.getMember().getEffectiveName()`.
